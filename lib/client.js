...
MqttClient.prototype._cleanUp = function (forced, done) {
  ...
  if (forced) {
    // If the user disabled automatic reconnections, it has to explicitly call 'reconnect'. When
    // that happens, if the 'clean' flag was set or we are using the internal store implementation,
    // the outgoing store will be in a clean state. This means that any outstanding callbacks will
    // never be called, because their ack won't match anything in the store, so we flush them here.
    if (this.options.reconnectPeriod === 0 && (this.options.clean || !this.options.outgoingStore)) {
...
/**
 * _flush - Invoke (if it exists) all of the callbacks in an outgoing queue with an
 *          error that we've disconnected
 * @api private
 */
MqttClient.prototype._flush = function (queue) {
  if (queue) {
